...
(function () {
    ...
    .provider('fsIcon', function() {
        ...
        this.$get = function($timeout, $mdDialog, $q) {
    ...
    .controller('fsIconCtrl',function($scope, $mdDialog, fsIcons, $timeout) {
        ...
        for (var i = 0, len = fsIcons.length; i < len; i++) {
            $scope.icons.push({ icon: fsIcons[i], name: fsIcons[i].replace(/_/g,' ') });
        }
        ...
        $scope.$watch('search',function(search) {
            ...
            if(search.toLowerCase().length) {
                for (var i = 0, len = $scope.icons.length; i < len; i++) {
                    if($scope.icons[i].icon.indexOf(search.toLowerCase())<0) {
                        $scope.hideIcons[$scope.icons[i].icon] = true;
                    }
                }
            }
        });
...
angular.module('fs-angular-icon').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/icon.html',
    ...
    "\n" +
    "            <div ng-repeat=\"icon in icons track by $index\" class=\"icon-item\" ng-click=\"select(icon)\" ng-hide=\"hideIcons[icon.icon]\">\r" +
    "\n" +
    "                <div class=\"material-icons\">{{::icon.icon}}</div>\r" +
    "\n" +
    "                <div class=\"tooltip\">{{::icon.name}}</div>\r" +
    "\n" +
    "            </div>\r" +
    "\n" +
    "        </div>\r" +
